...
Definition = ["Linear classifiers (SVM, logistic regression, etc.) with SGD training.<\br>This estimator implements regularized linear models with stochastic gradient descent (SGD) learning: the gradient of the loss is estimated each sample at a time and the model is updated along the way with a decreasing strength schedule (aka learning rate). SGD allows minibatch (online/out-of-core) learning via the partial_fit method. For best results using the default learning rate schedule, the data should have zero mean and unit variance.<\br>This implementation works with data represented as dense or sparse arrays of floating point values for the features. The model it fits can be controlled with the loss parameter; by default, it fits a linear support vector machine (SVM).<\br>The regularizer is a penalty added to the loss function that shrinks model parameters towards the zero vector using either the squared euclidean norm L2 or the absolute norm L1 or a combination of both (Elastic Net). If the parameter update crosses the 0.0 value because of the regularizer, the update is truncated to 0.0 to allow for learning sparse models and achieve online feature selection."]
Parameter_0 = {"Name":"loss", "Type": ["option"], "Default":"hinge", "Possible":['hinge', 'log_loss', 'log', 'modified_huber', 'squared_hinge', 'perceptron', 'squared_error', 'huber', 'epsilon_insensitive', 'squared_epsilon_insensitive'],
              "Definition":"The loss function to be used.<\br><\br>‘hinge’ gives a linear SVM.<\br><\br>‘log_loss’ gives logistic regression, a probabilistic classifier.<\br><\br>‘modified_huber’ is another smooth loss that brings tolerance to<\br><\br>outliers as well as probability estimates.<\br><\br>‘squared_hinge’ is like hinge but is quadratically penalized.<\br><\br>‘perceptron’ is the linear loss used by the perceptron algorithm.<\br><\br>The other losses, ‘squared_error’, ‘huber’, ‘epsilon_insensitive’ and ‘squared_epsilon_insensitive’ are designed for regression but can be useful in classification as well; see SGDRegressor for a description."}
...
Parameter_12 = {"Name":"learning_rate", "Type": ["option"], "Default":"optimal", "Possible":['adaptive', 'invscaling', 'optimal', 'constant'],
              "Definition":"The learning rate schedule:<\br><\br>‘constant’: eta = eta0<\br><\br>‘optimal’: eta = 1.0 / (alpha * (t + t0)) where t0 is chosen by a heuristic proposed by Leon Bottou.<\br><\br>‘invscaling’: eta = eta0 / pow(t, power_t)<\br><\br>‘adaptive’: eta = eta0, as long as the training keeps decreasing. Each time n_iter_no_change consecutive epochs fail to decrease the training loss by tol or fail to increase validation score by tol if early_stopping is True, the current learning rate is divided by 5."}
...
Parameter_19 = {"Name":"warm_start", "Type": ["bool"], "Default":False, "Possible":[True,False],
              "Definition":"When set to True, reuse the solution of the previous call to fit as initialization, otherwise, just erase the previous solution. <\br><\br> Repeatedly calling fit or partial_fit when warm_start is True can result in a different solution than when calling fit a single time because of the way the data is shuffled. If a dynamic learning rate is used, the learning rate is adapted depending on the number of samples already seen. Calling fit resets this counter, while partial_fit will result in increasing the existing counter."}
...
Definition = ["A random forest classifier.<\br><\br>A random forest is a meta estimator that fits a number of decision tree classifiers on various sub-samples of the dataset and uses averaging to improve the predictive accuracy and control over-fitting. The sub-sample size is controlled with the max_samples parameter if bootstrap=True (default), otherwise the whole dataset is used to build each tree."]
...
Parameter_6 = {"Name":"max_features", "Type": ["option"], "Default":"sqrt", "Possible":["sqrt","log2",None],
              "Definition":"The number of features to consider when looking for the best split:<\br><\br>If None, then max_features=n_features.<\br><\br>If “log2”, then max_features=log2(n_features).<\br><\br>If “sqrt”, then max_features=sqrt(n_features)."}
...
Parameter_17 = {"Name":"max_samples", "Type": ["int_or_null"], "Default":None, "Possible":["int"],
              "Definition":"If bootstrap is True, the number of samples to draw from X to train each base estimator.<\br><\br> If None (default), then draw X.shape[0] samples.<\br><\br>If int, then draw max_samples samples."}

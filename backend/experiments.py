...
def load_possible_experiments():
    ...
    g.section_Choices = [["PCA: Principal Component Analysis", "None"], ["PCA: Principal Component Analysis", "None"]]
    g.section_Info = [["PCA: Info", "None: No preoptimization will be performed"], ["PCA: Info", "None: No preoptimization will be performed"]]
    ...
    g.section_Info = [[None], ["k-Fold: Please give one number for the number of folds", 
...
def run_experiment_1():    
    ...
    # K-Fold
        ...
        kf = KFold(n_splits=int(data['Validation_Option']), shuffle=True, random_state = 1)
        kf.get_n_splits(X)
        ...
        for i, (train_index, test_index) in enumerate(kf.split(X)):
            ...
            disp.plot()
            temp = os.getcwd()
            plt.savefig(temp + '\\backend\\static\\Images\\' + fileName + "_fold_" + str(i) + '.png')
        ...
        # Send the Metrics
        Metrics = {"acc_list": acc_list.tolist(), 

...
def Split(data):
    ...
    train_set, test_set = train_test_split(df, test_size=float(data[data['validation'] + "_Input"]), random_state=1, shuffle=True)
...
def K_Fold(data):
    ...
    kf = KFold(n_splits=int(data[data['validation'] + "_Input"]), shuffle=True, random_state = 1)
    kf.get_n_splits(X)
